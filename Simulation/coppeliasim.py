...
def extract_features(sim, is_grasped):
    """Extract features from the simulation environment."""
    ...
    # Retrieve positions of key objects and joints
...
def prepare_input_features(features, feature_shape):
    """Prepare feature matrix for the model, filling unknown timesteps with 0."""
...
def check_ball_in_box(sim, box_handle, ball_handle):
    ...
    return ball_pos >= box_min and ball_pos <= box_max

def run_simulation(trials=100):
    """Run the simulation multiple times to evaluate the success rate."""
    ...
    for trial in range(trials):
        ...
        def prevent_collision_with_bounding_boxes_x(sim, box_handle, ball_handle):
            """Check if the new positions cause bounding boxes to overlap on the x-axis."""
            # Get the bounding box parameters for the box
            box_min_x = sim.getObjectFloatParam(box_handle, sim.objfloatparam_objbbox_min_x)
            box_max_x = sim.getObjectFloatParam(box_handle, sim.objfloatparam_objbbox_max_x)

            # Get the bounding box parameters for the ball
        ...
        while True:
            ...
            if prevent_collision_with_bounding_boxes_x(sim, box_handle, ball_handle):
                # If they don't overlap, break the loop and keep the positions
                break

            # If they overlap, reset to the original positions and try again
        ...
        is_grasped = False
        while True:
            features = extract_features(sim, is_grasped)
            ...
            if abs(np.sum(restored_predictions - old_restored_predictions)) < 1 and is_grasped:
                counter += 1
            else:
                counter = 0
        ...
        sim.stopSimulation()
        time.sleep(1)  # Ensure the simulation has time to stop before restarting
