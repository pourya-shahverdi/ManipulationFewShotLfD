...
models = {
    # 'grasp': {
    #     'LeftElbowYaw': keras.models.load_model('../model/best_model_LeftElbowYaw_grasp.keras'),
    #     'LeftElbowRoll': keras.models.load_model('../model/best_model_LeftElbowRoll_grasp.keras'),
    #     'LeftShoulderRoll': keras.models.load_model('../model/best_model_LeftShoulderRoll_grasp.keras'),
    #     'LeftShoulderPitch': keras.models.load_model('../model/best_model_LeftShoulderPitch_grasp.keras')
    # },
    # 'drop': {
    #     'LeftElbowYaw': keras.models.load_model('../model/best_model_LeftElbowYaw_drop.keras'),
    #     'LeftElbowRoll': keras.models.load_model('../model/best_model_LeftElbowRoll_drop.keras'),
    #     'LeftShoulderRoll': keras.models.load_model('../model/best_model_LeftShoulderRoll_drop.keras'),
    #     'LeftShoulderPitch': keras.models.load_model('../model/best_model_LeftShoulderPitch_drop.keras')
    # },
    'all': {
...
feature_shape = (3, len(feature_columns))  # window size, number of features
...
def extract_features(sim):
    ...
    base_2d_origin = np.array([0.295, -0.06, -0.3])
    base_3d_origin = np.array([0.0, 0.2, -1.65])
    ...
    data = [
        ...
        480 - (base_2d_origin[1] + box_pos[2] + 0.03) * object_y_ratio,
...
def run_simulation():
    ...
    csv_path = r'.\NaoPushingData\2024-08-09_19-11-58\data.csv'
    ...
    for index, row in list(df.iterrows())[::10]:
        ...
        ball_handle = sim.getObject('/Cuboid')
        ...
        if distance < 0.08:  # Threshold for "close" in meters
            ...
            print("Stopped!")
            ball_pos[0] -= 0.01
            sim.setObjectPosition(ball_handle, list(ball_pos))
    ...
    while True:
        ...
        print('features', features[feature_columns].values.astype(np.float64))
        ...
        for joint, model in models["all"].items():
            # for joint, model in models["grasp" if not is_grasped else "drop"].items():
            prediction = model.predict([prepared_features, np.array([25]), np.array([1 if is_grasped else -1])])
            predictions[joint] = prediction[0][n]

        restored_predictions = restore_scale(predictions, label_ranges)
        print('restored_predictions', restored_predictions)
...
# }
#
# for joint_handle, angle in angles_arm.items():
