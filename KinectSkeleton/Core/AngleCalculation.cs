...
{
    public static Vector3 FindShoulder(Vector3 shoulder, Vector3 elbow, Hand hand = Hand.Right)
    {
    ...
    public static float FindHeadPitch(Vector3 leftEye, Vector3 rightEye, Vector3 nose)
    {
        var vecBaseline = new Vector3(0.0034141643f, 0.7978445f, -0.60285366f);

        var midEye = (leftEye + rightEye) / 2;

        var vecNew = Vector3.Normalize(nose - midEye);

        return CalculateAngle(vecBaseline, vecNew, "ZYX").Z;
    }

    public static float FindHeadYaw(Vector3 leftEye, Vector3 rightEye)
    {
        var vecBaseline = new Vector3(-0.9985791f, -0.00043155675f, -0.053287648f);

        var vecNew = Vector3.Normalize(rightEye - leftEye);
        
        return -CalculateAngle(vecBaseline, vecNew, "ZYX").Y;
    }
